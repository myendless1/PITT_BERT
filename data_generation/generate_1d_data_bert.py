...
def generate_data_combined_equation(experiment: str, pde: dict, modes: list, num_samples: int, batch_size: int,
                                    alpha, beta, gamma, As, omegas, phis, ls,
    ...
    for mode in modes:
        ...
        for key in pde:
            ...
            h5f_u[key] = dataset.create_dataset(key, (num_samples, pde[key].grid_size[0], pde[key].grid_size[1]),
                                                dtype=float)
...
def generate_data_single_equation(h5f, experiment: str, pde: dict, modes: list, num_samples: int, batch_size: int,
                                  alpha, beta, gamma, As, omegas, phis, ls, all_tokens, encoded_tokens,
                                  device: torch.cuda.device = "cpu") -> None:
    """
    Generate data for combined equation using different coefficients
    Args:
    ...
    for mode in modes:
        ...
        for key in pde:
            ...
            lower_token = [i.lower() for i in all_tokens]
            lower_token=''.join(lower_token)
            tokens = tokenizer(lower_token)
            encoded_tokens = [tokens['input_ids'], tokens['token_type_ids'], tokens['attention_mask']]
    ...
    for (n, alpha, bet, gamma) in [("Heat", 0, 0.2, 0)]:
        for beta in [0.01, 0.05, 0.1, 0.2, 0.5, 1.]:
            ...
            for i in tqdm(range(NUM_RUNS)):
                ...
                pde = {}
                all_tokens = get_all_tokens(pde_tokens, delta_tokens, ic_tokens, left_bc, right_bc, As, omegas, ls,
                                            phis)
                ...
                pde[f'pde_{nt}-{nx}'] = CE(starting_time, end_time, grid_size=(nt, nx), L=L, alpha=alpha, beta=beta,
                                           gamma=gamma, device="cpu")
                single_exp_names = []
                exp_name = generate_data_single_equation(h5f, name, pde, [name], 1, 1, alpha, beta, gamma, As, omegas,
                                                         phis, ls,
            ...
            for beta in [0.01, 0.05, 0.1, 0.2, 0.5, 1.]:
                ...
                for i in tqdm(range(NUM_RUNS)):
                    ...
                    pde[f'pde_{nt}-{nx}'] = CE(starting_time, end_time, grid_size=(nt, nx), L=L, alpha=alpha, beta=beta,
                                               gamma=gamma, device="cpu")
                    single_exp_names = []
                    exp_name = generate_data_single_equation(h5f, name, pde, [name], 1, 1, alpha, beta, gamma, As,
                                                             omegas, phis, ls,
                                                             all_tokens, encoded_tokens)
