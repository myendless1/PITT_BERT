...
class LinearAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, pos=None, not_assoc=False, padding_mask=None):
        ...
        queries, keys, values = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h=self.heads),
                                    (queries, keys, values))
...
class PhysicsInformedTokenTransformerBert(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, num_heads, output_dim, neural_operator, bert_model,
        ...
        self.hidden_embedding_linear = nn.Linear(1024, hidden_dim, bias=False)
    ...
    def forward(self, queries, keys, values, t, device):  # , mask):
...
class StandardPhysicsInformedTokenTransformer(nn.Module):
    ...
    def forward(self, queries, keys, values, t):  # , mask):
        ...
        # Encoding/Embedding
        input_ids = keys[0]
        token_type_ids = keys[1]
        attention_mask = keys[2]
        ...
        bert_embedding = bert_embedding['hidden_states'][0]
